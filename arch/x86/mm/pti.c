		  ...
		  enum asi_clone_level level)
{
	...
	err = asi_clone_pgd_range(&pti_pgt, &init_mm.pgt,
				  start, end, level);
	WARN_ON(err);
}
...
static void __init pti_clone_user_shared(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		err = asi_map_page(&pti_pgt, va, pa, PAGE_KERNEL);
		if (WARN_ON(err))
			return;
	}
}
...
void __init pti_init(void)
{
	...
	pti_pgt.pgd = kernel_to_user_pgdp(init_mm.pgt.pgd);
