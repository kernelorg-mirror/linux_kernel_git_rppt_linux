...
static int ibmveth_open(struct net_device *netdev)
{
	...
	rc = -ENOMEM;
	adapter->buffer_list_addr = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!adapter->buffer_list_addr) {
		netdev_err(netdev, "unable to allocate list pages\n");
		goto out;
	}

	adapter->filter_list_addr = kzalloc(PAGE_SIZE, GFP_KERNEL);
...
out_free_filter_list:
	kfree((void *)(unsigned long)adapter->filter_list_addr);
out_free_buffer_list:
	kfree((void *)(unsigned long)adapter->buffer_list_addr);
out:
