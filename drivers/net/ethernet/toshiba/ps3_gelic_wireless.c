...
static int gelic_wl_do_wep_setup(struct gelic_wl_info *wl)
{
	...
	wep = kmalloc(PAGE_SIZE, GFP_KERNEL);
	if (!wep)
		return -ENOMEM;
...
static int gelic_wl_do_wpa_setup(struct gelic_wl_info *wl)
{
	...
	pr_debug("%s: <-\n", __func__);
	/* we can assume no one should uses the buffer */
	wpa = kmalloc(PAGE_SIZE, GFP_KERNEL);
	if (!wpa)
		return -ENOMEM;
				  ...
				  struct gelic_wl_scan_info *bss)
{
	...
	/* do common config */
	common = kmalloc(PAGE_SIZE, GFP_KERNEL);
	if (!common)
		return -ENOMEM;
...
out:
	kfree((void *)(unsigned long)common);
	pr_debug("%s: ->\n", __func__);
	return ret;
}
