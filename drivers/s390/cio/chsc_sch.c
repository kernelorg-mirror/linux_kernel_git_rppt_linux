...
static int chsc_ioctl_start(void __user *user_area)
{
	...
	if (!css_general_characteristics.dynio)
		...
		return -EOPNOTSUPP;
	chsc_area = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!chsc_area)
		return -ENOMEM;
...
static int chsc_ioctl_on_close_set(void __user *user_area)
{
	...
	on_close_chsc_area = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!on_close_chsc_area) {
		ret = -ENOMEM;
...
static int chsc_ioctl_info_channel_path(void __user *user_cd)
{
	...
	scpcd_area = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!scpcd_area)
		return -ENOMEM;
	cd = kzalloc_obj(*cd);
	if (!cd) {
...
static int chsc_ioctl_info_cu(void __user *user_cd)
{
	...
	scucd_area = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!scucd_area)
		return -ENOMEM;
	cd = kzalloc_obj(*cd);
	if (!cd) {
...
static int chsc_ioctl_info_sch_cu(void __user *user_cud)
{
	...
	sscud_area = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!sscud_area)
		return -ENOMEM;
	cud = kzalloc_obj(*cud);
	if (!cud) {
...
static int chsc_ioctl_conf_info(void __user *user_ci)
{
	...
	sci_area = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!sci_area)
		return -ENOMEM;
	ci = kzalloc_obj(*ci);
	if (!ci) {
...
static int chsc_ioctl_conf_comp_list(void __user *user_ccl)
{
	...
	sccl_area = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!sccl_area)
		return -ENOMEM;
	ccl = kzalloc_obj(*ccl);
	if (!ccl) {
...
static int chsc_ioctl_chpd(void __user *user_chpd)
{
	...
	scpd_area = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!scpd_area || !chpd) {
		ret = -ENOMEM;
		goto out_free;
	}
...
static int chsc_ioctl_dcal(void __user *user_dcal)
{
	...
	sdcal_area = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!sdcal_area)
		return -ENOMEM;
